...
from utils.logging_tools.log_controller import ERROR
...
class Cache:
    ...
    def __init__(self, filename: Union[Text, None]) -> None:
        ...
        if filename:
            self.path = ensure_path_sep("\\out_files\\cache\\" + filename)
        # 如果filename为None，则操作所有文件内容
        else:
            self.path = ensure_path_sep("\\out_files\\cache\\")
    ...
    def set_caches(self, value: Any) -> None:
        ...
        try:
            ...
            ERROR.logger.error("写入成功：{}".format(self.path))
    ...
    def clean_all_cache(cls) -> None:
        ...
        :return:
        """
        cache_path = ensure_path_sep("\\out_files\\cache\\")
...
class CacheHandler:
    ...
    @staticmethod
    def add_to_cache_list(cache_name, value):
        if cache_name not in _cache_config:
            _cache_config[cache_name] = []
        _cache_config[cache_name].append(value)
