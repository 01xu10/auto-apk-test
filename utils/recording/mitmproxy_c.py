...
class Counter:
    ...
    def __init__(self, ignore_host: tuple, keywords: tuple, filename: Text = './data/proxy_data.yaml'):
        self.num = 0
        self.file = filename
        self.counter = 1
        # 需要过滤的 host
    ...
    def is_ignore_host(self, host_name, keywords):
        if host_name in self.ignore_host:
            return False
        for kw in keywords:
            if kw in host_name:
                return False
